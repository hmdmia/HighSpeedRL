...
stop_no_improvement_callback = StopTrainingOnNoModelImprovement(max_no_improvement_evals=30, verbose=0)

target = RandomMovingTarget(target_location, v_range=(0, 0), psi_range=(0, 0), distribution="beta")
...
def optimize_agent(trial):
    ...
    log_path = 'tmp/tuning/movingTarget/PPO' + str(trial.number + 1)
    ...
    for delta_velocity, max_psi in zip(velocity_curriculum, psi_curriculum):
        ...
        model.learn(total_timesteps=int(500_000), callback=eval_callback)
        mean_reward, std_reward = evaluate_policy(model, env, n_eval_episodes=100)
        model_goodness += (mean_reward - std_reward) * (successful_trials + 1)
        if mean_reward < 150:
            print(f'Mean Reward = {mean_reward} < {continue_threshold}, breaking after v_max = {delta_velocity}, psi_max = {max_psi}...')
            break
        else:
...
# study = joblib.load("tmp/tuning/movingTarget/study.pkl")
study.optimize(optimize_agent, n_trials=100)

best_params = study.best_params
print(best_params.items())
save_hyperparams(best_params, "saved_hyperparams/movingTarget")

joblib.dump(study, "tmp/tuning/movingTarget/study.pkl")

...
def load_continuous(file_path, ppo_env):
    ...
    ppo_model = SAC.load('tmp/ppo_tmp', ppo_env)
    return ppo_model
...
if __name__ == '__main__':
    ...
    def train_model(total_observations, previous_model=None, tensorboard_log=None):
        ...
        if not previous_model:
            training_model = SAC('MlpPolicy', training_env, verbose=1, tensorboard_log=tensorboard_log)
        else:
            training_model = SAC.load(previous_model, env=training_env)

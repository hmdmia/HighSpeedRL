...
if __name__ == '__main__':
    ...
    # Define parameters
    target_velocity = np.array([1_000, 0, np.deg2rad(0)])  # [v, gam, psi] [m/s, rad, rad]
    ...
    def train_model(total_observations, previous_model=None, tensorboard_log=None):
        ...
        if not previous_model:
            training_model = PPO('MlpPolicy', training_env, verbose=1, **hyperparams,
                             use_sde=True, tensorboard_log=tensorboard_log)
        else:
            training_model = PPO.load(previous_model, env=training_env)
